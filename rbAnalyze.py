...
def showExampleGraph():
    ...
    p1 = plt.bar(ind, resourceData, width, color=colors[2], bottom=tuple(sum(t) for t in zip(randomData, disperseData, contractData)))
    p2 = plt.bar(ind, contractData, width, color=colors[0], bottom=tuple(sum(t) for t in zip(randomData, disperseData)))
    ...
    plt.xticks(ind + width/2., ("5-10", "10-15", "15-20", "20-25", "25-30"), fontsize=globalFont-2)
    ...
    #plt.legend(handles=patches, loc=7, bbox_to_anchor=(1.225, 0.5), fontsize=globalFont) #center right
    plt.legend((p2[0], p3[0], p1[0], p4[0]), ("Contract", "Dispersal", "Resource Collection", "Random Motion"), loc=7, bbox_to_anchor=(1.55, 0.5))


    #plt.show()
    plt.savefig("Graphics\\4Graphlet\\All\\Comparisons\\StackExample.png", bbox_inches="tight")
    plt.clf()
...
def saveStatGraph(stats, title, fileName):
    ...
    fileName = os.path.basename(fileName)
    dirToSave = dirToSave.replace(fileName, "Comparisons\\With Legends\\" + fileName).replace(".txt", ".png")
...
def generateLineGraphs():
    #k-graphlet
    gxVals = [1,5,10,25,50,100,500,1000,5000,10000,20000] #k-graphlet
    gAcc = [0.33,0.55,0.60,0.66,0.71,0.73,0.79,0.81,0.84,0.84,0.86] #Accuracy
    gSVM = [0.08,0.33,0.71,2.03,5.12,11.77,134.32,199.90,238.33,246.89,240.23]
    gFeats = [0.98,1.22,1.57,2.55,4.17,7.40,33.26,65.88,331.97,655.03,1350.31]
    ...
    mFeats = [90.51,90.33,90.37,88.35,88.20,89.04,89.27,87.50,88.00,88.40]
